...
class ConvMotionFunction(Function):
    ...
    def forward(ctx, input, mag, ori):
        output = custom_conv.conv_motion_forward(input, mag, ori)
    ...
    def backward(ctx, grad_out):
        ...
        grad_in = custom_conv.conv_motion_backward(grad_out, mag, ori, mcos, msin)
...
class ConvClsFunction(Function):
    ...
    def forward(ctx, input, labels, labels_unique, weight, bias):
        output = custom_conv.conv_cls_forward(input, labels, weight, bias)
    ...
    def backward(ctx, grad_out):
        ...
        grad_in = custom_conv.conv_cls_backward1(grad_out, labels, weight)
        if ctx.needs_input_grad[3]:
            grad_f = custom_conv.conv_cls_backward2(grad_out, input, labels, weight, labels_unique)
...
class InvConvMotionFunction(Function):
    @staticmethod
    def forward(ctx, input, labels, labels_unique, weight, bias):
        output = custom_conv.inv_conv_motion_forward(input, labels, weight, bias)
        ctx.save_for_backward(input, labels, labels_unique, weight, bias)
        return output
    ...
    def backward(ctx, grad_out):
        input, labels, labels_unique, weight, bias = ctx.saved_tensors
        grad_in = custom_conv.inv_conv_motion_backward(grad_out, labels, weight)
...
class LineFunction(Function):
    ...
    def forward(ctx, input, labels, weightx, weighty):
        output = custom_conv.line_forward(weightx, weighty, input, labels)
    ...
    def backward(ctx, grad_out):
        ...
        grad_in = custom_conv.line_backward1(weightx, weighty, input, grad_out, labels)
        if ctx.needs_input_grad[3]:
            grad_y = custom_conv.line_backward2(weightx, weighty, input, output, labels)
